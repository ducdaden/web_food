...
namespace foodie.User
{
    public partial class Payment : System.Web.UI.Page
    {
        ...
        void OrderPayment(string name, string cardNo, string expiryDate, string cvv, string address, string paymnetMode)
        {
            int paymentId;
            int productId;
            int quantity;
            ...
            cmd.Parameters.AddWithValue("@PaymentMode", paymnetMode);
            ...
            try
            {
                ...
                while (dr.Read())
                {
                    ...
                    //update product qtt end

                    //Delete cart item
                    DeleteCartItem(productId,transaction,con);
                    //delete cart item end
                    dt.Rows.Add(Utils.GetUniqueId(), productId, quantity,
                        (int)Session["userId"], "Pending", paymentId, Convert.ToDateTime(DateTime.Now));
                }
                dr.Close();
                #endregion Getting Cart Items's
            ...
            catch (Exception e)
            {
                ...
                {
                    Response.Write("<script>aler('" + ex.Message + "')</script>");
                }
        ...
        void UpdateQuantity(int _productId, int _quantity, SqlTransaction sqlTransaction, SqlConnection sqlConnection)
        {
            ...
            {
                Response.Write("<script>aler('" + e.Message + "')</script>");
            }
